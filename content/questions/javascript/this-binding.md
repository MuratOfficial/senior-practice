...
followUps:
  - q: "Что вернёт bind, применённый к уже связанной функции?"
    a: "Повторный bind не переопределяет this — первое связывание жёсткое и побеждает. Поменять можно лишь предустановленные аргументы, но не контекст."
  - q: "Как ведёт себя this в классовых полях-стрелках и чем это грозит при наследовании?"
    a: "Стрелка-поле захватывает this экземпляра — удобно для колбэков, но метод создаётся на каждом инстансе (не на прототипе), его нельзя переопределить через super в подклассе и он не виден как метод прототипа."
  - q: "Почему setTimeout(obj.method, 0) теряет контекст и как это чинить?"
    a: "Передаётся сама функция без объекта — при вызове this уже не obj (в strict — undefined). Чинят через obj.method.bind(obj) или стрелку () => obj.method()."
applications:
  - "Корректная передача методов в колбэки и обработчики (bind, стрелки)."
  - "Диагностика потери контекста в event listeners, таймерах, промисах."
  - "Проектирование API: стрелочные поля vs методы прототипа."
references:
  - title: "MDN: this"
