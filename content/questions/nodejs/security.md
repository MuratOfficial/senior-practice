...
followUps:
  - q: "Что такое prototype pollution и как она возникает из невинного merge?"
    a: "Рекурсивный merge/set по пути из пользовательского ввода может записать в __proto__ или constructor.prototype, добавив свойство всем объектам — отсюда обход проверок, DoS, иногда RCE. Защита — блокировать ключи __proto__/prototype, Object.create(null), Map, схемы."
  - q: "Как защититься от supply chain атак через npm-зависимости?"
    a: "Лок-файл и npm ci, аудит (npm audit, Snyk/Dependabot), минимум зависимостей, проверка мейнтейнеров и всплесков версий, pinning, provenance/подписи, задержка перед мажорами. Секреты не держать в переменных, доступных пакетам."
  - q: "Почему eval/child_process с пользовательским вводом — критично, и какие есть безопасные альтернативы?"
    a: "Это прямое исполнение кода/команд — инъекция даёт RCE. Никогда не конкатенируют ввод в eval или в shell. Альтернативы: execFile/spawn с массивом аргументов (без shell), белые списки, парсеры вместо eval; vm недостаточен для недоверенного кода."
applications:
  - "Защита от prototype pollution в парсерах и merge пользовательских данных."
  - "Управление рисками зависимостей (audit, lock, Dependabot) в CI."
  - "Безопасный запуск подпроцессов (execFile с аргументами, без shell-инъекции)."
references:
  - title: "OWASP Top 10"
