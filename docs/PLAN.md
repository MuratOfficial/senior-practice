...
- [x] Структурные ответы на follow-up + блок «Где применяется»: формат v2 (`followUps: [{q,a}]`, `applications: string[]`), Prisma `followUps Json` + миграция `20260722110000`, рендер follow-up под спойлером `<details>` (самопроверка сохранена). Сконвертированы **все 87 вопросов** во всех 10 темах: у каждого follow-up есть эталонный ответ, у каждого вопроса — блок «Где применяется»
