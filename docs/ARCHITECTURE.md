...
Challenge и LearningPath устроены так же: таблица с уникальным slug, вложенные структуры (языки и тест-кейсы задач, секции треков) — JSONB с Zod-валидацией на сиде. Треки описываются в `content/paths/*.json`; прогресс по треку выводится из пользовательских данных (ReviewState для вопросов, PASSED-сабмишены для задач) — отдельного стейта у треков нет.
